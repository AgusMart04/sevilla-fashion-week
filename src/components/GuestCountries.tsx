...
export function GuestCountries() {
  ...
  return (
    <section id="paises" className="relative bg-background py-24 md:py-32">
      <div className="mx-auto max-w-[1600px] px-6 md:px-12">
        <Reveal>
          ...
          <h2 className="font-display text-5xl md:text-7xl silver-text max-w-3xl text-balance">
            27 países. <span className="italic">Una</span> pasarela.
          </h2>
        ...
        <div className="mt-16 grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 xl:grid-cols-9 gap-px bg-border">
          {countries.map((c, i) => (
            <Reveal key={c.name} delay={i * 30}>
              <button
                type="button"
                onClick={() => setSelected(c)}
                className="group relative h-full w-full bg-background px-4 py-5 flex flex-col items-center justify-center text-center transition-all duration-700 hover:bg-card min-h-[110px] cursor-pointer"
              >
                <img
                ...
                {c.city && (
                  <span className="mt-0.5 text-[9px] text-silver">
                    {c.city}
                  </span>
                )}
                <span className="absolute bottom-0 left-0 h-px w-0 bg-silver transition-all duration-700 group-hover:w-full" />
              </button>
            </Reveal>
          ))}
        </div>
            ...
            <>
              ...
              <div className="flex flex-col items-center gap-4 py-4">
                ...
                <div className="text-center">
                  ...
                  <p className="mt-1 text-xs text-silver">{selected.region}</p>
                  {selected.city && (
